...
const dateBasicsQueryTpl = ctx => `
    __cdb_basics AS (
        SELECT
            ...
            ((date_trunc('${ctx.aggregation}', min(${ctx.column})) - interval \'${ctx.offset} hours\') at time zone 'UTC')::timestamptz AS __cdb_start_date,
...
const dateHistogramQueryTpl = ctx => `
    SELECT
        ...
        END AS bin,
        date_part('epoch', date_trunc('${ctx.aggregation}', min(${ctx.column})) AT TIME ZONE '${ctx.offset}')::numeric AS timestamp,
