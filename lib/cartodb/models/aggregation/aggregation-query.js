...
const gridResolution = ctx => {
    ...
    return `GREATEST(${256/ctx.res} * !pixel_width!, ${minimumResolution})::double precision`;
};
...
const spatialFilter = `
  (_cdb_query.the_geom_webmercator && _cdb_params.bbox)
`;
...
_cdb_params AS (
    SELECT  res,
            ...
            ST_SetSRID(ST_MakeBox2D(
                ...
                ST_Point(
                    ST_XMAX(bbox) - eps * res,
                    ST_YMAX(bbox) - eps * res
                )
            ), ST_SRID(_cdb_res.bbox)) as tr_bbox
    FROM _cdb_res
)
`;
...
const defaultAggregationQueryTemplate = ctx => `
    ...
    _cdb_clusters AS (
        ...
        WHERE ${spatialFilter}
        GROUP BY
            Round(ST_X(_cdb_query.the_geom_webmercator)/_cdb_params.res),
            Round(ST_Y(_cdb_query.the_geom_webmercator)/_cdb_params.res)
            ${dimensionNames(ctx)}
    ) SELECT
        _cdb_query.*
...
const aggregationQueryTemplates = {
    'centroid': ctx => `
        ...
        FROM _cdb_params, (
            SELECT *,
                  ST_SnapToGrid(the_geom_webmercator, ST_XMIN(bbox), ST_YMIN(bbox), res, res) as _geo_snapped
            FROM _cdb_params, (${ctx.sourceQuery}) _cdb_snap_query
            WHERE the_geom_webmercator && _cdb_params.tr_bbox
        ) _cdb_query
        GROUP BY
            _geo_snapped
            ${dimensionNames(ctx)}
    ...
    'point-grid': ctx => `
        ...
        _cdb_clusters AS (
            SELECT
            MIN(_cdb_query.cartodb_id) AS cartodb_id,
            Round(ST_X(_cdb_query.the_geom_webmercator)/_cdb_params.res)::int AS _cdb_gx,
            Round(ST_Y(_cdb_query.the_geom_webmercator)/_cdb_params.res)::int AS _cdb_gy
            ${dimensionDefs(ctx)}
            ${aggregateColumnDefs(ctx)}
            FROM (${ctx.sourceQuery}) _cdb_query, _cdb_params
            WHERE ${spatialFilter}
